    ...
    for k in range(nt-1):
        ...
        for i in range(3):
            ...
            for j in range(3):
                ...
                r[i][k+1][j]=r[i][k][j]+h*v[i][k][j]+((h**2)/2)*a[i][j]
                
        #exist to ensure get the full r vector the places it in a2   
        a2=f(r[0,k+1,:],r[1,k+1,:],r[2,k+1,:])
        
        #then comes back to v
        
        #planets
        for i in range(3):
            #compents
            for j in range(3):               
                v[i][k+1][j]=v[i][k][j]+h*((a[i][j]+a2[i][j])/2)
...
r,v,nt=verlet(t0,tf,h,doon,rv0)
...
Tn=['Earth(Velocity-Verlet)','Mars(Velocity-Verlet)']  #title name
fign=[['E_vv_postions','M_vv_postions'],['E_vv_velocity','M_vv_velocity']]   #figure names
...
plt.title('Earth,Mars-Velocity-Verlet',fontsize=16)
plt.grid()
plt.legend(fontsize=16)
plt.savefig('EMvvxy_plot.png', dpi=250)
